...
export default function routes(app, passport) {
	...
	// request REQUIRES: 1) JWT token in Authorization Header
	//				 	 2) email
	app.get('/settings/:email?', passport.authenticate('jwt', { session: false }),
		(req, res) => {
			console.log('/settings');
			console.log('You are authorized, ' + req.query.email);
